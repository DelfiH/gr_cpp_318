...
void virtual_elevator() {
    ...
    while (true) {
        ...
        if (!(std::cin >> chosen_level)) {
            std::cout << "Ошибка: Пожалуйста, введите число от 0 до 7." << std::endl;
        ...
        std::cout << "\nВы хотите покинуть торговый центр? (Y/N): ";
        std::cin >> exit_choice;

        // Приводим введенный текст к нижнему регистру
        std::string lower_exit_choice = toLower(exit_choice);

        if (lower_exit_choice == "y") {
            std::cout << "\nДо свидания!" << std::endl;
            break;
        } else if (lower_exit_choice == "n") {
            continue; // Возвращаемся к выбору этажа
        } else {
